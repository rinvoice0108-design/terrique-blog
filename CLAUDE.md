...
5. `knowledge/awkward-expressions.json` — 실제 발행 글에서 나온 어색한 한국어 사례 (관리자가 terrique-custom 화면에서 계속 추가함)
6. `output/_index.json` — 최근 사용한 패턴/도입부 확인 → **의도적으로 다른 조합 선택**
7. (수치 인용 시) `knowledge/conversion-benchmarks.md`

> `brand-facts.md`가 placeholder 상태(`[PLACEHOLDER]`로 시작)면 먼저 사용자에게 `/setup` 실행을 안내하고 멈출 것.

**`awkward-expressions.json` 사용법 — banned-words와 다르게 문자열 매치가 아니라 판단으로 적용할 것:**
이 파일의 각 항목은 "이 문장이 왜 어색한지"를 설명한 실제 사례다. 항목에 적힌 문자열 그대로가 다시 나올까봐 피하라는 게 아니라, **`note`에 적힌 어색함의 원인(관용구 일부 생략, 문맥에 안 맞는 단어 선택 등)을 이해하고, 같은 종류의 실수를 이번 글에서 저지르지 않는 것**이 목적이다. 글을 다 쓴 뒤에는 반드시 이 파일의 각 사례를 염두에 두고 본문을 다시 읽으며 비슷한 어색함이 있는지 스스로 검토하고 자연스럽게 고칠 것.

지금까지 쌓인 사례들의 공통점은 **문장에서 무엇을 가리키는지(목적어·지시 대상)가 빠져서** 어색해졌다는 것이다. "갸웃하셨다면"(무엇을 갸웃?), "진짜 상대입니다"(무엇의 상대?), "여기서 드러납니다"(무엇이 드러남?)처럼 관용구를 일부만 쓰거나 지시어를 던져놓고 대상을 생략하면 독자가 앞뒤 문맥을 되짚어야 한다. 문장을 쓸 때마다 "이 문장만 따로 읽어도 무엇에 대한 말인지 알 수 있는가"를 스스로 점검할 것.

#### 글쓰기 원칙

- ⚠️ **`facts.json`의 `confidential: true` 항목은 `text`를 그대로 쓰지 말 것.** 반드시 같은 항목의 `public_alt` 표현으로 바꿔 쓴다. 제조공장 상호·소재지(예: 인도/베트남 공장명)는 **거래 관계상 대외 비공개**다. 인증을 언급할 때는 공장명 대신 **인증번호와 시험기관명**을 병기한다. `quality-check.js`의 "대외 비공개 정보 유출" 항목이 이를 검사한다
...
⚠️ "고개를 끄덕이다/갸웃하다" 같은 관용구는 "고개를"을 생략하지 말 것 — "갸웃하셨다면 잘 오셨습니다"처럼 줄이면 무엇이 갸웃했는지 빠져 한국어로 어색하다. `quality-check.js`의 "관용구 일부 생략" 항목이 이를 검사한다.
...
`--subject`(핵심 주제 한 줄)와 `--points`(구체적 포인트 최소 2개)는 **필수**입니다. 비어 있으면 프롬프트 내용이 키워드 한 단어로 붕괴해 매번 비슷한 이미지가 나오므로, 스크립트가 즉시 에러로 종료합니다.

`assets/product-master.jpg`(실제 제품 사진 1장)가 있으면 5장 전부에 참조 이미지로 자동 첨부돼 제품 일관성이 유지됩니다.

```bash
GEMINI_API_KEY=your_key node scripts/generate-images.js \
  --title "글 제목" \
  --keyword "키워드" \
  --subject "핵심 주제 한 줄 (예: 면 수건을 올바른 주기로 세탁하고 관리하는 일상 장면)" \
  --points "포인트1|||포인트2|||포인트3" \
  --output "output/폴더/images"
```

생성 이미지 5종 (모두 사진 전용 — 텍스트/타이포그래피 없음, `knowledge/image-axes.json`으로 앵글·조명·배경을 글마다 다르게 로테이션):
1. **01-hero** (16:9) — 히어로 사진
2. **03-detail** (1:1) — 텍스처 클로즈업
3. **04-product** (2:3) — 제품/배치 플랫레이
4. **05-scene** (4:3) — 결과 장면
5. **07-closing** (3:4) — 무드 클로징
...
> 2026-08-03 브랜드 리뉴얼("컬러풀 클래식") 반영, 2026-08-04 갱신. `/setup-tone`이 2026-06-08에 수집한 원본 샘플(`knowledge/tone-samples/real-blog-posts.txt`)은 리뉴얼 이전 "호텔의 품격" 톤이라 문장 리듬·구조(도입부 패턴, 짧은 줄바꿈, 직접 말걸기)만 참고하고, 아래 핵심 가치·시그니처 표현을 우선 적용할 것. 리뉴얼 이후 실제 발행 글이 쌓이면 `/setup-tone`을 다시 실행해 샘플을 갱신할 것.

**브랜드 핵심가치**: 컬러풀 클래식 (Colorful Classic) — 가장 익숙한 클래식 디자인에 다채로운 컬러 플레이를 더하는 것
**도입부**: 브랜드 인사 → 독자 일상 공감 → 문제 제기 순서로 시작 (구조는 유지, 문구만 교체)
**문장 스타일**: 짧은 줄바꿈으로 리듬감. 독자에게 직접 말걸기 ("~해보셨을 겁니다")
**핵심 가치**: 컬러풀 클래식, 클래식, 다채로움, 색감, 감각적인 일상, 세련됨, 나만의 색

**시그니처 표현** (글마다 1~2개 자연스럽게 삽입):
- "변하지 않는 클래식에, 다채로운 색감을 더하다"
- "지루한 일상에 기분 좋은 색을 더해보세요"
- "테리크와 함께 컬러풀 클래식을 완성해보세요"
- "매일 마주하는 수건 하나에도 나만의 색을 담아보세요"
- "클래식한 디자인에 어울리는 컬러를 고르는 즐거움"

**실제 차별점(수치 근거, `facts.json` own-017~019 참고)**: 호텔수건 40수 200g 라인 14색 보유 — 컬러 다양성을 추상적으로 말하지 말고 이 수치로 구체화할 것

**마무리**: 브랜드 제품 특징 1~2줄 → 따뜻한 권유형 클로징 ("~해보세요")

⚠️ "집에서 누리는 호텔의 품격" 계열 옛 시그니처는 브랜드 리뉴얼로 방향이 달라져 더 이상 기본 사용하지 않음. 호텔급 스펙 자체(예: 200g 라인, 코마사)를 설명할 때는 여전히 사실로서 언급 가능하나, 브랜드 정체성 표현으로는 위 컬러풀 클래식 시그니처를 우선할 것.
